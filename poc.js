...
(async function () {
  ...
  // In the shell, we can call db.getMongo() to get the redacted connection
  // string for the current cluster. We can try to match that to the right cluster name
  // so users don't need to select the cluster they want to attach the service to
  const getMongoResult = process.env.getMongoResult;
  const currentClusterUrl = new URL(getMongoResult);
  ...
  // Get clusters
  const clusters = await getClusters({ groupId: groups[0] });
  logger.info(clusters.map(c => c.name));

  // Find the right cluster
  const cluster = clusters.find(c => {
    const srvUrl = new URL(c.srvAddress);
    return srvUrl.hostname === currentClusterUrl.hostname;
  });
